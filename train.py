...
def train():
    ...
    ssd_net =ssd.build_ssd("train",cfg['min_dim'], cfg['num_classes'])
    net = ssd_net
    ...
    if args.visdom:
        vis_title = 'SSD.PyTorch on ' + dataset_train.name
        vis_legend = ['Loc Loss', 'Conf Loss', 'Total Loss']
        iter_plot = create_vis_plot('Iteration', 'Loss', vis_title, vis_legend)
        epoch_plot = create_vis_plot('Epoch', 'Loss', vis_title, vis_legend)

    # 利用DataLoader实现了批量处理、打乱、多线程加载等功能
    data_loader = data.DataLoader(dataset_train, args.batch_size,num_workers=args.num_workers,
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        # torch.Size([32, 8732, 4]) torch.Size([32, 8732, 2]) torch.Size([8732, 4])
