...
def point_form(boxes):
    ...
    """
    ret = torch.cat((boxes[:, :2] - boxes[:, 2:]/2,boxes[:, :2] + boxes[:, 2:]/2), 1)
    return ret
...
def jaccard(box_a, box_b):
    #box_a is truths, box_b is priors
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    #找出与每个ground——truth对应的iou最大的priorbox
    best_prior_overlap, best_prior_idx = overlaps.max(1, keepdim=True)
    # 将每一个真实框对应的最佳PriorBox的IoU设置为2
    best_truth_overlap, best_truth_idx = overlaps.max(0, keepdim=True)
    #每一个priorbox在所有真实框中最大的
...
def encode(matched, priors, variances):
    ...
    # change base on issue https://github.com/amdegroot/ssd.pytorch/issues/162
